...
def test_rf011_sigue_bloqueando_aunque_forzar_true(client, db_session, autenticar_como):
    ...
    body = response.json()
    assert "mensajes" in body["detail"]
    assert any(
        "32" in mensaje or "máximo" in mensaje.lower()
        for mensaje in body["detail"]["mensajes"]
    )
