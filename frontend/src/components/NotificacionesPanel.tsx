    ...
    <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="max-h-80 overflow-y-auto">
        {NOTIFICACIONES.length === 0 && (
          <p className="px-4 py-6 text-center text-sm text-slate-500 dark:text-slate-400">No tienes notificaciones.</p>
        )}
        {GRUPOS.map((grupo) => {
          ...
          return (
            <div key={grupo}>
              <p className="px-4 pt-3 pb-1 text-[11px] font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                {grupo}
              </p>
              {items.map((n) => (
                <div key={n.id} className="flex items-start gap-2.5 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700/60">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="shrink-0 text-[11px] text-slate-500 dark:text-slate-400">{n.hora}</span>
                    </div>
