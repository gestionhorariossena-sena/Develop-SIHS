...
const estiloTipo: Record<TipoNotificacion, string> = {
  cruce: 'bg-orange-50 text-orange-600 dark:bg-orange-950/50 dark:text-orange-300',
  horario: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-300',
  ambiente: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300',
  sistema: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300',
}
    ...
    <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 dark:border-slate-700">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="cursor-not-allowed text-xs font-medium text-slate-400 dark:text-slate-500"
          >
            Marcar leídas
          </button>
          <button
            ...
            onClick={onCerrar}
            title="Cerrar"
            className="flex h-5 w-5 items-center justify-center rounded text-slate-400 hover:bg-slate-50 hover:text-slate-600 dark:text-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-300"
          >
      ...
      <div className="max-h-80 overflow-y-auto">
        {NOTIFICACIONES.length === 0 && (
          <p className="px-4 py-6 text-center text-sm text-slate-400 dark:text-slate-400">No tienes notificaciones.</p>
        )}
        {GRUPOS.map((grupo) => {
          ...
          return (
            <div key={grupo}>
              <p className="px-4 pt-3 pb-1 text-[11px] font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">
                {grupo}
              </p>
              {items.map((n) => (
                <div key={n.id} className="flex items-start gap-2.5 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700/60">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline justify-between gap-2">
                      <p className={`text-sm ${n.leida ? 'font-medium text-slate-700 dark:text-slate-300' : 'font-semibold text-slate-900 dark:text-slate-100'}`}>
                        {n.titulo}
                      </p>
                      <span className="shrink-0 text-[11px] text-slate-400 dark:text-slate-400">{n.hora}</span>
                    </div>
                    <p className="mt-0.5 line-clamp-2 text-xs text-slate-500 dark:text-slate-400">{n.descripcion}</p>
                  </div>
                </div>
              ))}
            </div>
