...
export function ExportarPdfButton({ etiqueta = 'Exportar a PDF', className }: ExportarPdfButtonProps) {
  return (
    <button
      ...
      className={`${
        className ??
        'rounded-xl border border-outline px-4 py-2 text-sm font-semibold text-on-surface-variant hover:bg-surface dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700'
      } print:hidden`}
    >
      {etiqueta}
    </button>
