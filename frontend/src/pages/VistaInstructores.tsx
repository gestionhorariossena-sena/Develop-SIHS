...
export function VistaInstructores() {
  ...
  return (
    <AppShell activo="Vista por instructores">
      <div className="mb-6">
        <h1 className="mb-1 text-2xl font-bold text-on-surface dark:text-slate-100">Vista por instructores</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-[20rem_1fr]">
        <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-4 dark:border-slate-700 dark:bg-slate-800" aria-label="Filtro de instructores">
          <label htmlFor="buscar-instructor-vista" className="mb-1.5 block text-xs font-medium text-on-surface-variant dark:text-slate-400">
            Buscar instructor
          </label>
          <input
            ...
            className="mb-3 w-full rounded-xl border border-outline bg-surface-container-lowest px-3 py-2 text-sm text-on-surface placeholder:text-on-surface-variant focus:border-sena-600 focus:ring-1 focus:ring-sena-600 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
          />

          <div className="mb-3 grid grid-cols-2 gap-2">
            <div>
              <label htmlFor="filtro-ficha-vista" className="mb-1.5 block text-xs font-medium text-on-surface-variant dark:text-slate-400">
                Ficha
              </label>
              <select
                ...
                className="w-full rounded-xl border border-outline bg-surface-container-lowest px-3 py-2 text-sm text-on-surface-variant dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
              >
            ...
            <div>
              <label htmlFor="filtro-ambiente-vista" className="mb-1.5 block text-xs font-medium text-on-surface-variant dark:text-slate-400">
                Ambiente
              </label>
              <select
          ...
          {cargando ? (
            <p className="py-6 text-center text-sm text-on-surface-variant dark:text-slate-400">Cargando instructores…</p>
          ) : (
            ...
            <>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-on-surface dark:text-slate-100">{seleccionado.nombre}</p>
                    <p className="text-sm text-on-surface-variant dark:text-slate-400">{seleccionado.email}</p>
                  </div>
                </div>
                <Link
