...
export function VistaFichas() {
  ...
  return (
    <AppShell activo="Vista por fichas">
      <div className="mb-6">
        <h1 className="mb-1 text-2xl font-bold text-on-surface dark:text-slate-100">Vista por fichas</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-[20rem_1fr]">
        <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-4 dark:border-slate-700 dark:bg-slate-800" aria-label="Filtro de fichas">
          <label htmlFor="buscar-ficha-vista" className="mb-1.5 block text-xs font-medium text-on-surface-variant dark:text-slate-400">
            Buscar ficha
          </label>
          <input
            ...
            className="mb-3 w-full rounded-xl border border-outline bg-surface-container-lowest px-3 py-2 text-sm text-on-surface placeholder:text-on-surface-variant/70 focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
          />

          <div className="mb-3">
            <label htmlFor="filtro-instructor-vista" className="mb-1.5 block text-xs font-medium text-on-surface-variant dark:text-slate-400">
              Instructor
            </label>
            <select
              ...
              className="w-full rounded-xl border border-outline bg-surface-container-lowest px-3 py-2 text-sm text-on-surface dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
            >
          ...
          {cargando ? (
            <p className="py-6 text-center text-sm text-on-surface-variant dark:text-slate-400">Cargando fichas…</p>
          ) : (
            <ul className="max-h-[28rem] space-y-1 overflow-y-auto">
                  ...
                  <li key={ficha.idFicha}>
                    <button
                      ...
                      className={`flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-left text-sm transition ${
                        esActivo
                          ? 'bg-primary-container text-primary dark:bg-sena-950/50 dark:text-sena-300'
                          : 'text-on-surface hover:bg-surface dark:text-slate-300 dark:hover:bg-slate-700'
                      }`}
                    >
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sena-100 text-xs font-bold text-primary dark:bg-sena-950/50">
              ...
              {visibles.length === 0 && (
                <p className="px-2.5 py-6 text-center text-sm text-on-surface-variant dark:text-slate-400">Sin resultados.</p>
              )}
            </ul>
          )}
        </section>
            ...
            <>
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-full bg-sena-100 font-bold text-primary dark:bg-sena-950/50">
                    {seleccionada.codigoFicha.slice(0, 2).toUpperCase()}
                  </span>
                  <div>
                    <p className="font-semibold text-on-surface dark:text-slate-100">{seleccionada.codigoFicha}</p>
                    <p className="text-sm text-on-surface-variant dark:text-slate-400">
